...
import com.administrator.yaya.bean.ParametersInfo;
import com.administrator.yaya.jiajun.AES;
...
import static com.administrator.yaya.broadcast.NetworkUtils.isNetworkConnected;
...
class NetInterceptor {
    ...
    public OkHttpClient getClientWithCache() {
        return new OkHttpClient.Builder()
                .cache(new Cache(new File(BaseApp.getInstance().getCacheDir(), "NetCache10"), 10 * 1024 * 1024))
                .connectTimeout(20, TimeUnit.SECONDS)
                .readTimeout(20, TimeUnit.SECONDS)
                .writeTimeout(20, TimeUnit.SECONDS)
                .addInterceptor(getLogInterceptor())
                .addInterceptor(new CommonHeadersInterceptor())
                .addInterceptor(cacheInterceptor(BaseApp.getApplication()))
                .addNetworkInterceptor(cacheInterceptor(BaseApp.getApplication()))
                .sslSocketFactory(NetTrustManager.getNetTrustManager().createSSLSocketFactory()).hostnameVerifier(new NetTrustManager.TrustAllHostnameVerifier())
                .build();
    }
